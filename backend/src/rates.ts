...
type RateApiResponse = { result: string; rates: Record<string, number> };

async function fetchLiveRates(): Promise<Record<string, number>> {
  // Public free endpoint, base INR
  const res = await fetch('https://open.er-api.com/v6/latest/INR');
  if (!res.ok) throw new Error(`Rate fetch failed with status ${res.status}`);
  const json = (await res.json()) as Partial<RateApiResponse>;
  if (json?.result !== 'success' || !json?.rates) throw new Error('Invalid rate payload');
  return json.rates;
}
